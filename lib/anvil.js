var ArgParser, builder, colors, cssminifier, dive, emitter, events, express, fs, gzipper, jslint, jsp, mkdir, path, pro, resource, _;
  ...
  Configuration.prototype.ensurePaths = function(onComplete, prefix) {
    ...
    if (_.isObject(config.output)) {
      ...
      return fp.ensurePath(config.output, function() {
        return fp.ensurePath(config.tmp, function() {
          return onComplete();
        });
      });
    }
...
var Compiler, coffeeKup, coffeeScript, haml, less, marked, scss, stylus;
...
var ImagePipeline, MarkupPipeline, SourcePipeline, StylePipeline;
...
Anvil = (function() {
  function Anvil(config, fp, compiler, combiner, scheduler, log) {
    ...
    this.steps = {
      ...
      markupReady: function() {
        return (source || !hasSource) && (style || !hasStyle);
      },
      allDone: function() {
        return (source || !hasSource) && (style || !hasStyle) && (markup || !hasMarkup);
      }
    };
  }
  ...
  Anvil.prototype.buildMarkup = function() {
    var combiner, compiler, findPatterns, replacePatterns, scheduler, self;
    self = this;
    ...
    return this.prepFiles("markup", function(list) {
      return scheduler.parallel(list, compiler.compile, function() {
        return combiner.combine(list, function() {
          return self.stepComplete("markup");
        });
      });
    });
  };
  Anvil.prototype.buildSource = function() {
    ...
    combiner = new this.combiner(this.fp, scheduler, findPatterns, replacePatterns);
    return this.prepFiles("source", function(list) {
      return scheduler.parallel(list, compiler.compile, function() {
        return combiner.combine(list, function() {
          return self.stepComplete("source");
        });
      });
    });
  };
  Anvil.prototype.buildStyle = function() {
    ...
    combiner = new this.combiner(this.fp, scheduler, findPatterns, replacePatterns);
    return this.prepFiles("style", function(list) {
      return scheduler.parallel(list, compiler.compile, function() {
        return combiner.combine(list, function() {
          return self.stepComplete("style");
        });
      });
    });
  };
  ...
  Anvil.prototype.report = function() {
    return onComplete("Hey, it's done, bro-ham");
  };
