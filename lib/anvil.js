var ArgParser, builder, colors, dive, emitter, events, express, fs, mkdir, path, resource, _;
...
log = new Log();
exports.log = log;
var Configuration, config, continuous, debug, ext, extensionLookup, inProcess, libConfig, path, quiet, siteConfig, test, version, _;
_ = require("underscore");
path = require("path");
config = {};
siteConfig = {
...
var Compiler, coffeeKup, coffeeScript, haml, less, marked, scss, stylus, _;
...
StylePipeline = (function() {
  ...
  StylePipeline.prototype.process = function(files, onComplete) {
    ...
    return this.scheduler.parallel(minified, this.minify, function() {
      return onComplete(files.concat(minifed));
    });
  };
...
SourcePipeline = (function() {
  ...
  SourcePipeline.prototype.process = function(files, onComplete) {
    ...
    return this.scheduler.parallel(files, this.finalize, function() {
      return self.scheduler.parallel(minified, self.minify, function() {
        return self.scheduler.parallel(minified, self.finalize, function() {
...
PostProcessor = (function() {
  ...
  PostProcessor.constructor = function(config, fp, scheduler, log) {
    var uglify;
    this.config = config;
    this.fp = fp;
    this.scheduler = scheduler;
    ...
    this.style = new StylePipeline(this.config, this.fp, cssmin, this.scheduler, this.log);
...
var Anvil;
Anvil = (function() {
  function Anvil(config, fp, compiler, combiner, scheduler, postProcessor, log) {
    this.config = config;
    this.fp = fp;
      ...
      return self.moveFiles(list, function() {
        return combiner.combineList(list, function() {
          return scheduler.parallel(list, compiler.compile, function() {
            return postProcessor.process(list, function(list) {
...
exports.run = function() {
  var compiler, configuration, parser, postProcessor;
