...
var buildTarget, config, conventionConfig, ensurePaths, ext, importRegex, loadConfig, loadConvention, prepConfig, writeConfig, _base;
config = {};
conventionConfig = {
  "source": "src",
  "output": "lib",
...
loadConfig = function() {
  ...
  return readFile("./" + buildTarget, function(x) {
    config = JSON.parse(x);
...
buildTarget = ((_base = global.process.argv)[2] || (_base[2] = "build")) + ".json";
...
exports.run = function() {
  ...
  if (args[2] === "template") {
    output = args[3] || (args[3] = "build.json");
    return writeConfig(output);
  } else {
    onStep("Checking for config...");
    return path.exists("./build.json", function(exists) {
      return prepConfig(exists);
    });
  }
};
process = function() {
