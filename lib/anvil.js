var ArgParser, builder, colors, emitter, events, express, fs, mkdir, path, _;
...
log = new Log();
exports.log = log;
var Configuration, config, continuous, debug, defaultMocha, ext, extensionLookup, inProcess, libConfig, path, quiet, siteConfig, test, version, _;
_ = require("underscore");
path = require("path");
config = {};
siteConfig = {
...
Configuration = (function() {
  ...
  Configuration.prototype.configure = function(onConfig) {
    var buildFile, buildOpt, createLibFile, createSiteFile, exists, host, htmlPage, libScaffold, name, scaffold, self, showVersion, siteScaffold, type;
    self = this;
  ...
  Configuration.prototype.normalizeConfig = function(onComplete) {
    var calls, finalize, fp, outputPath, self, spec, wrap;
    self = this;
    fp = this.fp;
    ...
    spec = config.spec;
    if (spec && !config.mocha && !config.qunit) {
      calls.push(function(done) {
        return fp.getFiles(spec, function(files) {
          var specFile;
          if (files.length === 0) {
            return done();
          } else {
            ...
            if (!specFile) {
              ...
              try {
                return fp.read(specFile, function(content) {
                  var hasQUnit;
      ...
      return fp.getFiles(prereqPath, function(files) {
        ...
        copy = function(file, done) {
          ...
          if (!fp.pathExists([config.ext, fileName])) {
            return fp.move(file, [config.ext, fileName], done);
          } else {
            return done();
          }
        };
...
var FSProvider, fp, fs, _;
fs = require("fs");
_ = require("underscore");
FSProvider = (function() {
  function FSProvider() {
    this.crawler = new FSCrawler(scheduler);
    _.bindAll(this);
  }
...
var Compiler, coffeeKup, coffeeScript, haml, less, marked, stylus, _;
...
Compiler = (function() {
  ...
  Compiler.prototype.compile = function(file, onComplete) {
    var ext, log, newExt, newFile, self;
    self = this;
    ext = file.ext();
    newExt = this.extensionMap[ext];
    newFile = file.name.replace(ext, newExt);
    log = this.log;
    return this.fp.transform([file.workingPath, file.name], this.compilers[ext], [file.workingPath, newFile], function(err) {
      if (!err) {
        file.name = newFile;
        return onComplete();
      } else {
...
var ImagePipeline, MarkupPipeline, PostProcessor, SourcePipeline, StylePipeline, cssminifier, gzipper, jslint, jsp, pro;
...
var Anvil;
Anvil = (function() {
  function Anvil(config, fp, compiler, combiner, scheduler, postProcessor, log, callback) {
    this.config = config;
    this.fp = fp;
  ...
  Anvil.prototype.build = function() {
    if (!this.inProcess) {
      ...
      return this.log.onError("NOOOOOO");
    }
  };
  ...
  Anvil.prototype.processType = function(type, findPatterns, replacePatterns) {
    var combiner, compiler, postProcessor, scheduler, self;
    self = this;
      ...
      return self.moveFiles(list, function() {
        return combiner.combineList(list, function() {
          return scheduler.parallel(list, compiler.compile, function() {
            ...
            return postProcessor[type].process(final, function(list) {
  ...
  Anvil.prototype.finalOutput = function(files, onComplete) {
    ...
    return forAll(files, move, onComplete);
  };
  ...
  Anvil.prototype.prepFiles = function(type, onComplete) {
    var log, output, typePath, working;
...
var Context, MochaRunner, Runner, Suite, interfaces, mocha, reporters;
mocha = require("mocha");
...
Host = (function() {
  function Host(fp, scheduler, compiler, config) {
    ...
    app.get(/.*[.](coffee|kup|less|styl|md|markdown|haml)/, function(req, res) {
      ...
      return self.fp.read("." + req.url, function(content) {
...
exports.run = function() {
  var anvil, ci, compiler, configuration, fileChange, mochaRunner, parser, socketServer;
  ...
  return configuration.configure(function(config) {
    ...
    anvil = new Anvil(config, fp, compiler, Combiner, scheduler, postProcessor, log, function() {
