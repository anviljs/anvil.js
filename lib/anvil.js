...
createWatch = function() {
  var continuous, dir, diver, onChange, _i, _len, _ref, _results;
  continuous = false;
  ...
  _ref = ['source', 'spec', 'ext'];
  _results = [];
  for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    dir = _ref[_i];
    _results.push(diver(dir));
  }
  return _results;
};
triggerProcess = function() {
  ...
  createPage();
  process();
  return console.log("process was triggered " + new Date());
};