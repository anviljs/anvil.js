...
configure = function() {
  var buildFile, buildOpt, buildTemplate, generator, host, htmlPage, output, parser, scaffold, showVersion;
...
var ClientNotifier, buildHead, buildScripts, clientNotifier, createPageFor, hostPavlov;
...
createPageFor = function(targetSource) {
  var extPath, libPath, specPath;
  specPath = config.spec || (config.spec = "./spec");
  extPath = config.ext || (config.ext = "./ext");
  libPath = targetSource || (targetSource = config.output || (config.output = "./lib"));
...
hostStatic = function() {
  var app, extPath, htmlPath, libPath;
  htmlPath = config.static = "./html";
  extPath = config.ext || (config.ext = "./ext");
  libPath = config.output || (config.output = "./lib");
  app = express.createServer();
