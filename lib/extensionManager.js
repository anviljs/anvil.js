...
var extensionManagerFactory = function( _, anvil ) {
	...
	ExtensionManager.prototype.getExtensions = function( done ) {
		...
		this.getExtensionList( function( extensions ) {
			var removals = [];
			anvil.scheduler.parallel( extensions, function( extension, done ) {
				...
				anvil.log.debug( "loading extensions took " + ( Date.now() - start ) + "ms" );
				done();
			} );
		} );
	};
	...
	ExtensionManager.prototype.loadExtension = function( extensionName, extensionPath, list, removals ) {
		...
		try {
			...
			if( loadTime > 5 ) {
				anvil.log.warning( "Extension at '" + modulePath + "' took more than " + loadTime + "ms to load." );
			}
		} catch ( err ) {
